...
*   **Frontend (Vue):**
    *   [ ] Реализовать плавные CSS-переходы (fade-in, scale-up) для модальных окон.
    *   [ ] Улучшить hover-эффекты на карточках и кнопках (легкое увеличение, тень, изменение цвета).
    *   [ ] Интегрировать библиотеку для конфетти (например, `canvas-confetti`) и вызывать анимацию, когда `is_completed` у `Item` становится `true`.
